...
def obtener_columnas_comunes(carpeta):
    try:
        archivos = [
            f for f in os.listdir(carpeta) if f.endswith(".xlsx") or f.endswith(".xls")
        ]
        if not archivos:
            return []
            ...
            try:
                ...
                for i, row in df.iterrows():
                    if not row.isnull().all():
                        df.columns = [
                            str(col).strip().lower() for col in row
                        ...
                        df = df.loc[
                            :, (df.columns != "")
                        ]  # También elimina columnas con nombre vacío
                        break
...
def procesar_archivos():
    ...
    archivos = [
        f
        for f in os.listdir(carpeta_seleccionada)
    ...
    for archivo in archivos:
        ...
        df = pd.read_excel(ruta, usecols=seleccionadas)
        if filtro_texto:
            df = df[
                df.apply(
...
style.map(
    "TButton",
    background=[("active", "#005bb5"), ("!active", "#0078D7")],
...
carpeta_label = ttk.Label(
    frame, text="📂 Ninguna carpeta seleccionada", foreground="black"
)
carpeta_label.pack()
...
destino_label = ttk.Label(
    frame, text="📁 Ningún destino seleccionado", foreground="black"
)
destino_label.pack()
btn_destino = ttk.Button(
    frame, text="📁 Seleccionar Destino", command=seleccionar_destino
)
btn_destino.pack()
