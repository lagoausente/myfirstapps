...
class FileRenamerApp:
    def __init__(self, root):
        ...
        tk.Button(root, text="Seleccionar", command=self.select_origin_folder).pack()

        tk.Label(root, text="Carpeta de destino:").pack()
        tk.Entry(root, textvariable=self.dest_dir, width=50).pack()
        tk.Button(root, text="Seleccionar", command=self.select_dest_folder).pack()
    ...
    def preview_changes(self):
        ...
        dest_folder = self.dest_dir.get()
        prefix = self.prefix.get()
        suffix = self.suffix.get()
        new_ext = self.new_extension.get()

        if not folder:
            messagebox.showerror("Error", "Selecciona una carpeta de origen primero.")
            return

        if not dest_folder:
            dest_folder = folder  # Si no se especifica destino, usar la misma carpeta
