            ...
            try:
                ...
                for i, row in df.iterrows():
                    if not row.isnull().all():
                        ...
                        df.columns = [str(col).strip().lower() for col in df.columns]  # Convertir nombres de columna a minúsculas
...
def procesar_archivos():
    ...
    for archivo in archivos:
        ...
        df.columns = [str(col).strip().lower() for col in df.columns]  # Convertir todas las columnas a minúsculas
        df = df.loc[:, ~df.columns.str.contains("^Unnamed", case=False)]  # Eliminar columnas "Unnamed"
